...
def run_system_validation() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="socratiq-qa-") as tmp:
        ...
        if retrieval and retrieval_items:
            base_tokens = retrieval._tokenize("graph expansion bm25")
            ...
            reranked = retrieval._rerank_candidates(ranked, scores, base_tokens)
        ...
        quiz_payload = quiz_chat.json().get("result", {}) if quiz_chat.ok else {}
        submit = client.post(
            "/quiz/submit",
            json={
        ...
        eval_cases = [EvalCase(query=c["query"], gold_node_ids=set(c["gold_node_ids"])) for c in cases]
